...
public:
    ...
    bool check_gameover() {
        if (_position.x < 0 || _position.x > SCREEN_W - size) return false;
        if (_position.y < 0 || _position.y > SCREEN_H - size) return false;
        
        for (int i = 0; i < len; i++) {
            if (trail[i] == _position) return false;
        }
        return true;
    }
    ...
    bool update() {
        ...
        if (!check_gameover()) return false;
        draw();
        return true;
    }
